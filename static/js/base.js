...
var hm_forth_data_url = base_url+"";
var hm_fifth_data_url = base_url+"screen/elec/data/";
var hm_sixth_data_url = base_url+"";

//guestnum
$.ajax({
        url:hm_first_data_url,
        type:'get',
        dataType:'json',
...
$.ajax({
        ...
        success:function (res) {
            temhum_date = res.data.date;
            hum_data = res.data.hum_data;
            tem_data = res.data.tem_data;
...
$.ajax({
        ...
        success:function (res) {
            xAxisData=res.data.date;
            data1=res.data.pm_data;
            data2=res.data.co2_data;
...
$.ajax({
        ...
        success: function (res) {
          ...
          var ganmao = res.data.ganmao;//最低温度
          var weather = minTemperature.split(' ')[1] + '~' + maxTemperature.split(' ')[1];
          var type = res.data.forecast[0].type;//天气状态
            $("#weatherImg").attr("src","../static/images/screen/weather/"+type+".png");
            $("#hm_weh").text(type);
            $("#hm_temf").text(weather);
            $("#hm_city").text(city);
            // console.log(ganmao);
            // $("#hm_w_text").text(ganmao);
        },
        error: function (err) {
    ...
    if (index == 0) {
        ...
        if ($(this).parent().data('city') == 1) {
            ...
            if ($("#barType").find('.active').data('value') == 1) {
                $('#titleQ').html('<span>全网</span>到北京');
            } else if ($("#barType").find('.active').data('value') == 2) {
                $('#titleQ').html('北京到<span>全网</span>')
            }
            $('#city').html(li_con);
            ...
            if ($("#barTypes").find('.active').data('value') == 1) {
                $('#titleQs').html('<span>全网</span>到北京');
            } else if ($("#barTypes").find('.active').data('value') == 2) {
                $('#titleQs').html('北京到<span>全网</span>')
            }
            $('#citys').html(li_con);
        }
    } else {
...
$('#city').on('click', 'li', function () {
    ...
    if ($("#barType").find('.active').data('value') == 1) {
        $('#titleQ').html('<span>' + $(this).html() + '</span>到北京');
    } else if ($("#barType").find('.active').data('value') == 2) {
        $('#titleQ').html('北京到<span>' + $(this).html() + '</span>')
    }
})
...
$('#citys').on('click', 'li', function () {
    ...
    if ($("#barTypes").find('.active').data('value') == 1) {
        $('#titleQs').html('<span>' + $(this).html() + '</span>到北京');
    } else if ($("#barTypes").find('.active').data('value') == 2) {
        $('#titleQs').html('北京到<span>' + $(this).html() + '</span>')
    }
})
...
function chart4(data, type, chartType) {
    var str = '<li><span></span><p>院校</p><p>人数</p></li>';
    for (var i = 0; i < 10; i++) {
...
function roseChartFun(){
    var roseChart = echarts.init(document.getElementById('rose-chart'));
    var roseChart1 = echarts.init(document.getElementById('totalProfits'));
    window.addEventListener('resize', function () {
        roseChart.resize();
        roseChart1.resize();
